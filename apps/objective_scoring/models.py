...
class ObjectiveScoringPolicy(models.Model):
    ...
    plans_weight = models.DecimalField(max_digits=4, decimal_places=2)
    ...
    class Meta:
        constraints = [
            ...
            models.CheckConstraint(check=models.Q(plans_weight__gte=0, plans_weight__lte=1), name="ck_plans_weight_range"),
            models.CheckConstraint(check=models.Q(pd_max_hours__gt=models.F("pd_target_hours")), name="ck_pd_cap_gt_target"),
            models.CheckConstraint(
                check=models.Q(plans_max_count__gt=models.F("plans_target_count")),
    ...
    def clean(self) -> None:
        total = Decimal(self.pd_weight) + Decimal(self.plans_weight)
        if total != Decimal("1.00"):
            raise ValidationError("pd_weight + plans_weight must equal 1.00")
