...
class ComparisonResultViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    serializer_class = ComparisonResultSerializer
    permission_classes = [IsTeacherLeaderOrAdmin]

    def get_queryset(self):
        user = self.request.user
        qs = ComparisonResult.objects.select_related("teacher", "teacher__user", "cycle")
